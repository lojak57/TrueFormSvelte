...
<div class="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-6">
  <Card class="w-full max-w-md">
    ...
    {#if error}
      <div class="mb-6 p-4 rounded-lg {error.includes('created') || error.includes('confirmation') ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}">
        <p class="text-sm {error.includes('created') || error.includes('confirmation') ? 'text-green-700' : 'text-red-700'}">
          {error}
        </p>
      </div>
    {/if}
    ...
    {#if !isSignupMode}
      <div class="mt-6 text-center">
        <a href="/reset-password" class="text-sm text-accent-600 hover:text-accent-700 transition-colors">
