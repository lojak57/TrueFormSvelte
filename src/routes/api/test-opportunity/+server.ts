...
export const POST: RequestHandler = async ({ request }) => {
  try {
    const sampleLead = {
      ...
      projectDescription:
        "We need a modern business website to showcase our services and attract new customers. Looking for a professional design with contact forms and service pages.",
      websiteType: "Business Website",
      features: ["Contact Forms", "Blog/News", "Social Media Integration"],
      colorPreferences:
        "Professional blues and whites, matching our existing brand",
    ...
    return json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to create test opportunity",
      },
      { status: 500 }
    );
  }
