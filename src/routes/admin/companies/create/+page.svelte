  ...
  async function handleSubmit(event: CustomEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.code === '23505' && errorData.details?.includes('tf_companies_name_key')) {
          throw new Error(`A company named "${companyData.name}" already exists. Please choose a different name.`);
        }
      ...
      errorMessage = error instanceof Error ? error.message : 'Failed to create company';
    } finally {
      isSubmitting = false;
    }
  }
