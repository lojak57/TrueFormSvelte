  ...
  try {
    ...
    try {
      ...
      company = companies.find(
        (c) => c.name.toLowerCase() === wizardData.businessName?.toLowerCase()
      );
...
• Font feel: ${
      Array.isArray(data.fontFeel)
        ? data.fontFeel.join(", ")
        : data.fontFeel || "Not specified"
    }
...
• Inspiration: ${data.inspirationLinks || "None provided"}`);
  ...
  // Working Style
  if (
    data.workingStyle !== undefined ||
    data.wantToTalk !== undefined ||
    data.wantMockup !== undefined
  ) {
...
• Base Package: $999 (Custom site + hosting + support)`;
    ...
    if (data.selectedAddons && data.selectedAddons.length > 0) {
      ...
      data.selectedAddons.forEach((addonId) => {
        const addonTitle =
          addonTitles[addonId as keyof typeof addonTitles] || addonId;
        scopingText += `\n  - ${addonTitle}`;
      });
    }
    ...
    if (data.estimatedTotal) {
      ...
      scopingText += `\n• Payment Structure: 25% deposit ($${Math.round(
        data.estimatedTotal * 0.25
      ).toLocaleString()}) + 75% completion ($${Math.round(
        data.estimatedTotal * 0.75
      ).toLocaleString()})`;
    }
...
• Completed: ${
    data.completedAt ? new Date(data.completedAt).toLocaleString() : submittedAt
  }`);
