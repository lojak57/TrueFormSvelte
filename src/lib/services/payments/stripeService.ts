  ...
  async createPaymentLink(options: PaymentLinkOptions): Promise<string> {
    ...
    try {
      ...
      const session = await this.stripe.checkout.sessions.create({
        ...
        success_url:
          successUrl ||
          `${process.env.PUBLIC_SITE_URL}/proposals/${proposal.id}/payment-success?session_id={CHECKOUT_SESSION_ID}`,
        cancel_url:
          cancelUrl ||
          `${process.env.PUBLIC_SITE_URL}/proposals/${proposal.id}/payment-cancelled`,
