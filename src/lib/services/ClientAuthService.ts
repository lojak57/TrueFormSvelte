...
> {
  ...
  async getCompanyClients(
    companyId: string
  ): Promise<Array<UserProfile & ClientPortalAccess>> {
    const { data, error } = await this.supabase
      .from("tf_client_portal_access")
      .select(
        `
  ...
  async getTeamCompanies(
    teamMemberId: string
  ): Promise<Array<Company & TeamCompanyAccess>> {
    const { data, error } = await this.supabase
      .from("tf_team_company_access")
      .select(
        `
  ...
  async getCompanyTeam(
    companyId: string
  ): Promise<Array<UserProfile & TeamCompanyAccess>> {
    const { data, error } = await this.supabase
      .from("tf_team_company_access")
      .select(
        `
  ...
  async createOrUpdateProfile(
    userId: string,
    profileData: Partial<CreateUserProfileDTO>
  ): Promise<UserProfile> {
  ...
  async getAccessLevel(
    ...
    ClientPortalAccess["access_level"] | TeamCompanyAccess["role"] | null
  > {
  ...
  async getInvitationStatus(companyId: string): Promise<
    Array<{
      email: string;
