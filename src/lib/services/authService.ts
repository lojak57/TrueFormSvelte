...
import type { UserSession, LoginCredentials, AuthResponse } from '$lib/types';
...
export async function signupUser({ email, password, confirmPassword }: SignupCredentials): Promise<AuthResponse> {
  try {
    // Validation
    if (!email || !password) {
      throw new AuthError('Email and password are required');
