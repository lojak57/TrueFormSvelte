...
describe('CTAButtons Component', () => {
  ...
  it('applies responsive visibility classes', () => {
    ...
    expect(buttonContainer).toHaveClass('hidden', 'md:flex', 'items-center', 'space-x-4');
  });
  ...
  it('renders buttons with correct variants and sizes', () => {
    ...
    expect(adminButton).toHaveClass('border', 'border-gray-300', 'bg-white', 'text-gray-900');
  ...
  it('applies custom styling classes', () => {
    ...
    expect(buttons[1]).toHaveClass('shadow-lg', 'hover:shadow-xl', 'transition-shadow', 'duration-300');
  });
  ...
  it('maintains semantic structure', () => {
    ...
    expect(outerDiv).toHaveClass('hidden', 'md:flex', 'items-center', 'space-x-4');
