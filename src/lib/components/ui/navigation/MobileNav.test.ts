...
describe('MobileNav Component', () => {
  ...
  it('applies correct mobile menu container styling', () => {
    ...
    expect(menuContainer).toHaveClass(
      ...
      'bg-white', 'border-t', 'border-gray-100', 'shadow-xl', 'z-40'
    );
  });
  ...
  it('renders services dropdown button for items with children', () => {
    ...
    expect(servicesButton).toHaveClass('flex', 'items-center', 'justify-between', 'w-full');
  ...
  it('applies hover effects correctly', () => {
    ...
    const aboutLink = screen.getByRole('link', { name: 'About' });
    expect(aboutLink).toHaveClass('hover:text-accent-600', 'transition-colors', 'duration-200');
  });
  ...
  it('applies correct spacing and layout classes', () => {
    ...
    const ctaSection = screen.getByRole('link', { name: /admin dashboard/i }).closest('.pt-4');
    expect(ctaSection).toHaveClass('pt-4', 'border-t', 'border-gray-100', 'space-y-3');
  });
}); 