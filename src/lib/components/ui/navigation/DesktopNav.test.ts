...
describe('DesktopNav Component', () => {
  ...
  it('shows service descriptions in dropdown', () => {
    ...
    expect(screen.getByText('Like our flagship vcsews.com')).toBeInTheDocument();
  ...
  it('applies hover effects to navigation links', () => {
    ...
    const aboutLink = screen.getByRole('link', { name: 'About' });
    expect(aboutLink).toHaveClass('hover:text-accent-600', 'transition-all', 'duration-200');
    
    // Should have hover underline effect
    const hoverUnderline = aboutLink.querySelector('.group-hover\\:scale-x-100');
  ...
  it('applies correct dropdown styling when open', () => {
    ...
    const dropdown = screen.getByText('Marketing Websites').closest('.absolute');
  ...
  it('renders service icons correctly', () => {
    ...
    const iconContainers = screen.getAllByRole('link').filter(link => 
      link.querySelector('.bg-accent-100')
    );
    ...
    iconContainers.forEach(container => {
      ...
      expect(iconDiv).toHaveClass('w-10', 'h-10', 'rounded-lg', 'flex', 'items-center', 'justify-center');
    });
  });
