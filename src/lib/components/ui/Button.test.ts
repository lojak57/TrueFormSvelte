...
describe('Button Component', () => {
  it('renders with default props', () => {
    ...
    expect(button).toHaveClass('inline-flex', 'items-center', 'justify-center', 'rounded-lg');
  });
  ...
  it('does not trigger click when disabled', async () => {
    const handleClick = vi.fn();
    const { component } = render(ButtonTest, { disabled: true, slotContent: 'Disabled' });
  ...
  it('supports all variant options', () => {
    ...
    variants.forEach((variant) => {
      const { unmount } = render(ButtonTest, { variant, slotContent: `${variant} button` });
  ...
  it('supports all size options', () => {
    ...
    sizes.forEach((size) => {
      const { unmount } = render(ButtonTest, { size, slotContent: `${size} button` });
  ...
  it('has proper focus styles', () => {
    ...
    expect(button).toHaveClass('focus:outline-none', 'focus:ring-2', 'focus:ring-accent-500');
  });
