<script lang="ts">
  ...
  onMount(() => {
    ...
    const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
      catchError(event.reason instanceof Error ? event.reason : new Error(String(event.reason)));
    };
      ...
      <div class="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
        ...
        {#if error}
          <details class="text-left mb-6">
            ...
            <pre class="mt-2 text-xs bg-gray-100 p-3 rounded overflow-auto text-gray-700">
              {error.message}
              {#if error.stack}
                {'\n\n' + error.stack}
              {/if}
        ...
        <div class="flex gap-3 justify-center">
          {#if showRetry}
            <Button variant="accent" on:click={retry} class="flex items-center gap-2">
              <RefreshCw size={16} />
              Try Again
            </Button>
          {/if}
    ...
    <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1">
          ...
          {#if showRetry}
            <Button size="sm" variant="outline" on:click={retry} class="flex items-center gap-2">
              <RefreshCw size={14} />
              Retry
            </Button>
          {/if}
