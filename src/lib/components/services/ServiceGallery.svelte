<script lang="ts">
  ...
  export let description: string = "Real examples of what we've built for businesses like yours.";
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {#each gallery as item, i}
            <div 
              ...
              tabindex="0"
            >
              <div class="relative rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 group-hover:scale-105">
                <!-- Loading Placeholder -->
                {#if !imageLoadStates[i]}
                  <div class="w-full aspect-[4/3] bg-gray-200 animate-pulse flex items-center justify-center">
                    <div class="w-8 h-8 border-2 border-accent-300 border-t-accent-600 rounded-full animate-spin"></div>
                  </div>
                {/if}
                
                <!-- Lazy Loaded Image -->
                <IntersectionObserver threshold={0.1} once={true} let:isVisible={imageVisible}>
                  {#if imageVisible}
                    <img
                      src={item.src}
                      alt={item.alt}
                      class="w-full aspect-[4/3] object-cover transition-transform duration-300 group-hover:scale-110 {imageLoadStates[i] ? 'opacity-100' : 'opacity-0'}"
                      loading="lazy"
