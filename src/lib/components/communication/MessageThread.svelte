<script lang="ts">
	...
	function setupRealtimeSubscription() {
		realtimeSubscription = messageService.subscribeToThread(thread.id, (payload) => {
	...
	async function handleRealtimeEvent(payload: any) {
		...
		if (table === 'tf_messages') {
			if (eventType === 'INSERT') {
				// New message added
				const messageWithDetails = await messageService.getMessageById(newRecord.id);
				...
				if (newRecord.sender_id !== currentUser.user_id) {
					await messageService.markMessageAsRead(newRecord.id, currentUser.user_id);
				}
			} else if (eventType === 'UPDATE') {
				// Message updated
				const updatedMessage = await messageService.getMessageById(newRecord.id);
	...
	async function updateTypingIndicators() {
		try {
			...
			typingUsers = typingUsers.filter(user => user.user_id !== currentUser.user_id);
		} catch (err) {
			console.warn('Failed to update typing indicators:', err);
		}
	}

	async function handleSendMessage(event: CustomEvent<{ content: string; replyTo?: string }>) {
		try {
	...
	async function handleReaction(event: CustomEvent<{ messageId: string; reactionType: string }>) {
		try {
			...
			if (existingReaction) {
				// Remove reaction
				await messageService.removeReaction(messageId, currentUser.user_id, reactionType);
			} else {
				// Add reaction
	...
	// Format thread title
	$: threadDisplayName = thread.title || `${thread.company?.name || 'Chat'} - ${thread.thread_type}`;
</script>
		...
		<div class="flex items-center justify-between">
			<div class="flex items-center min-w-0">
				{#if thread.company?.logo}
					...
					/>
				{:else}
					<div class="w-10 h-10 bg-blue-500 rounded-full mr-3 flex-shrink-0 flex items-center justify-center">
				...
				{/if}
				<div class="min-w-0">
					<h3 class="font-semibold text-gray-900 truncate">{threadDisplayName}</h3>
