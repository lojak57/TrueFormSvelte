<script lang="ts">
	...
	$: readByOthers = message.read_by?.filter(r => r.user_id !== currentUserId) || [];
...
<div class="flex {isOwn ? 'justify-end' : 'justify-start'} mb-4 group">
	...
	<div class="max-w-xs lg:max-w-md relative">
		...
		{#if message.reply_to_message}
			<div class="mb-2 px-3 py-2 bg-gray-100 rounded-lg text-xs text-gray-600 border-l-4 border-blue-400">
				<div class="font-medium">{message.reply_to_message.sender?.first_name || 'User'}</div>
					...
					{#each message.attachments as attachment}
						...
						>
							...
							<div class="flex-1 min-w-0">
								...
								<div class="text-gray-500">{formatFileSize(attachment.file_size)}</div>
							</div>
							<button
		...
		<!-- Message metadata -->
		<div
			class="flex items-center mt-1 {isOwn ? 'justify-end' : 'justify-start'} text-xs text-gray-500"
		>
		...
		<div class="absolute top-0 right-0 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white border border-gray-200 rounded-lg p-1 -mr-2 -mt-2">
			<!-- Quick reactions -->
