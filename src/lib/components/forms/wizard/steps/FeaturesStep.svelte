<script lang="ts">
  ...
  const FEATURE_TIERS = {
    base: {
      contact: {
        ...
        image:
          "https://images.unsplash.com/photo-1596526131083-e8c633c948d2?w=400&h=300&fit=crop&crop=center",
      },
      gallery: {
        name: "Photo Gallery",
        icon: "🖼️",
      ...
      seo: {
        name: "SEO & Search",
        icon: "🔍",
        description: "Get found & searched easily",
        image:
          "https://images.unsplash.com/photo-1432888622747-4eb9a8efeb07?w=400&h=300&fit=crop&crop=center",
      },
      analytics: {
        name: "Analytics Dashboard",
        icon: "📊",
        description: "Track visitors & performance",
        image:
      ...
      social: {
        ...
        image:
          "https://images.unsplash.com/photo-1611262588024-d12430b98920?w=400&h=300&fit=crop&crop=center",
      },
      mobile: {
      ...
      testimonials: {
        ...
        icon: "⭐",
        description: "Show reviews & ratings",
        image:
      ...
      maps: {
        ...
        image:
          "https://images.unsplash.com/photo-1524661135-423995f22d0b?w=400&h=300&fit=crop&crop=center",
      },
      forms: {
        name: "Custom Forms",
        icon: "📝",
        description: "Surveys & applications",
        image:
      ...
      calendar: {
        name: "Event Calendar",
        icon: "📆",
        description: "Display events & dates",
        image:
    ...
    premium: {
      blog: {
        ...
        price: 100,
        image:
          "https://images.unsplash.com/photo-1499750310107-5fef28a66643?w=400&h=300&fit=crop&crop=center",
      },
      chat: {
        name: "Live Chat",
        icon: "💬",
      ...
      members: {
        ...
        icon: "🔐",
        description: "Private member areas",
        price: 250,
        image:
  ...
  $: baseFeatureCount = selected.filter(
    (id) => id in FEATURE_TIERS.base
  ).length;
  $: premiumFeatureCount = selected.filter(
    (id) => id in FEATURE_TIERS.premium
  ).length;
...
<div class="space-y-6">
  ...
  >
    ...
    {#if remainingBase > 0}
      <p class="text-sm text-gray-600 mt-2">
        Choose {remainingBase} more feature{remainingBase !== 1 ? "s" : ""} at no
        extra cost
      </p>
    {:else if premiumFeatureCount > 0}
      <p class="text-sm text-accent-600 font-medium mt-2">
        <Sparkles size={14} class="inline" />
        {premiumFeatureCount} premium feature{premiumFeatureCount !== 1
          ? "s"
          : ""} selected
      </p>
    {/if}
        ...
        >
          ...
          <div class="relative h-32 overflow-hidden rounded-t-xl">
            <img
              src={feature.image}
              alt={feature.name}
              class="w-full h-full object-cover {selected.includes(id)
                ? 'brightness-110'
                : ''}"
            />
            <div
    ...
    <div class="grid grid-cols-2 gap-4">
      {#each Object.entries(FEATURE_TIERS.premium) as [id, feature]}
        ...
        >
          ...
          {:else}
            <span
              class="absolute top-2 right-2 px-2 py-1 bg-accent-100 text-accent-700 text-xs rounded-full z-10 font-semibold"
            >
              +${feature.price}
            </span>
          {/if}
  ...
  <div class="text-center text-sm text-gray-500 pt-4 border-t">
    <p>
      Don't worry - you can always add or remove features later from your
      dashboard!
    </p>
  </div>
</div>
