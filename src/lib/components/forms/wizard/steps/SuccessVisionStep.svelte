<script lang="ts">
  ...
  import { Target, TrendingUp, Users, DollarSign, Clock, Star, Rocket, Trophy } from 'lucide-svelte';
  ...
  const successMetrics = [
    {
      ...
      icon: TrendingUp,
      color: 'blue',
      benefits: ['Higher conversion rates', 'Better lead quality', 'Automated lead capture']
    },
    {
      id: 'increase-sales',
      ...
      icon: DollarSign,
      color: 'green',
      benefits: ['Streamlined checkout', 'Trust-building design', 'Persuasive product pages']
    },
    {
      id: 'save-time',
      ...
      icon: Clock,
      color: 'purple',
      benefits: ['Automated bookings', 'Self-service options', 'Reduced admin work']
    },
    {
      ...
      icon: Star,
      color: 'yellow',
      benefits: ['Professional appearance', 'Social proof display', 'Trust indicators']
    },
    {
      id: 'reach-audience',
      ...
      icon: Users,
      color: 'indigo',
      benefits: ['Better SEO rankings', 'Mobile optimization', 'Social media integration']
    },
    {
      id: 'compete-better',
      ...
      icon: Trophy,
      color: 'orange',
      benefits: ['Modern design edge', 'Better user experience', 'Unique positioning']
    },
    {
      id: 'launch-product',
      ...
      icon: Rocket,
      color: 'red',
      benefits: ['Launch-ready design', 'Scalable foundation', 'Market positioning']
    },
    {
      ...
      icon: Target,
      color: 'teal',
      benefits: ['Intuitive navigation', 'Fast loading times', 'Mobile-first design']
    }
  ];
...
<div class="space-y-8" in:fade={{ duration: 300 }}>
  ...
  <div class="text-center">
    <div class="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-green-500 to-blue-500 rounded-2xl mb-6 shadow-xl">
      <Target size={40} class="text-white" />
    </div>
    
    <div class="mb-8">
      <p class="text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed">
        Let's envision your success! What will this new website help you achieve? Pick up to 3 main goals.
      </p>
    </div>
  </div>
      ...
      >
        ...
        <div class="flex items-start gap-4 mb-4">
          <div class="flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center
            ...
            <svelte:component this={metric.icon} size={24} class="text-{metric.color}-600" />
          </div>
          
          <div class="flex-1">
            <h3 class="text-lg font-bold text-gray-900 mb-2 group-hover:text-{metric.color}-700 transition-colors">
              {metric.title}
            </h3>
        ...
        <div class="space-y-1">
          {#each metric.benefits as benefit}
            <div class="flex items-center gap-2 text-xs text-gray-500">
              <div class="w-1 h-1 bg-{metric.color}-400 rounded-full"></div>
              <span>{benefit}</span>
            </div>
          {/each}
        </div>
        ...
        {#if !selected.includes(metric.id) && selected.length >= 3}
          <div class="absolute inset-0 bg-gray-100 bg-opacity-75 rounded-2xl flex items-center justify-center">
            <span class="text-sm font-medium text-gray-500">3 goals selected</span>
          </div>
        {/if}
      </button>
    {/each}
  ...
  {#if selected.length > 0}
    <div class="bg-gradient-to-r from-green-50 to-blue-50 rounded-2xl p-6 max-w-4xl mx-auto border border-green-200"
      ...
      <div class="grid md:grid-cols-{selected.length} gap-4">
        {#each selectedMetrics as metric, i}
          <div class="text-center" in:fly={{ y: 20, duration: 300, delay: i * 100 }}>
            <div class="inline-flex items-center justify-center w-16 h-16 bg-white rounded-xl shadow-sm mb-3 border border-{metric.color}-200">
              <svelte:component this={metric.icon} size={24} class="text-{metric.color}-600" />
            </div>
      ...
      <div class="text-center mt-6 pt-6 border-t border-green-200">
        <p class="text-sm text-gray-600 italic">
          "In 6 months, our new website will be {selectedMetrics.map(m => m.title.toLowerCase()).join(', ').replace(/, ([^,]*)$/, ', and $1')}."
        </p>
      </div>
    </div>
  {/if}
  ...
  <div class="text-center pt-6">
    ...
    {:else}
      <div class="space-y-3">
        <p class="text-gray-600">Select at least one success goal to continue</p>
        <p class="text-sm text-gray-500">💡 Choose the outcomes that matter most to your business</p>
      </div>
    {/if}
  </div>
</div>
