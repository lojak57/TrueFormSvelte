...
<div class="premium-step" in:fade={{ duration: 400 }}>
  ...
  <div class="text-center mb-12">
    <div
      class="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-orange-500 to-amber-600 rounded-2xl mb-6 shadow-xl"
    >
    ...
    <p
      class="text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed font-light"
    >
      Select the aesthetic direction that resonates with your brand vision.
      Choose up to two for a distinctive blend.
    </p>
  </div>
    ...
    {#each vibes as vibe, i}
      ...
      >
        ...
        >
          ...
          <div
            class="relative bg-white rounded-2xl border-2 transition-all duration-300 hover:shadow-xl overflow-hidden
          ...
          >
            ...
            {#if selected.includes(vibe.id)}
              <div
                class="absolute inset-0 bg-gradient-to-br from-orange-50 to-amber-50 opacity-60"
              />
            {/if}
            ...
            {#if selected.includes(vibe.id)}
              <div class="absolute -top-3 -right-3 z-20">
                <div
                  class="w-8 h-8 bg-gradient-to-br from-orange-500 to-amber-600 rounded-full flex items-center justify-center shadow-lg"
                >
                  <span class="text-white font-semibold text-sm"
                    >{selected.indexOf(vibe.id) + 1}</span
                  >
                </div>
              </div>
            {/if}
            ...
            <div class="relative z-10 p-6">
              <!-- Mini Browser Window -->
              <div
                class="website-preview bg-gray-50 rounded-lg overflow-hidden border border-gray-200 mb-6"
              >
                <!-- Browser Chrome -->
                <div class="bg-gray-200 px-3 py-2 flex items-center gap-2">
                  <div class="flex gap-1.5">
                    <div class="w-2.5 h-2.5 bg-red-400 rounded-full" />
                    <div class="w-2.5 h-2.5 bg-yellow-400 rounded-full" />
                    <div class="w-2.5 h-2.5 bg-green-400 rounded-full" />
                  </div>
                  <div
                    class="flex-1 bg-white rounded px-2 py-1 text-xs text-gray-500 font-mono"
                  >
                    company.com
                  </div>
                </div>
                ...
                <!-- Website Content -->
                <div
                  class="p-4 {vibe.id === 'modern-minimal'
                    ? 'bg-slate-50'
                    : vibe.id === 'bold-executive'
                    ? 'bg-orange-50'
                    : vibe.id === 'refined-luxury'
                    ? 'bg-purple-50'
                    : vibe.id === 'trusted-professional'
                    ? 'bg-blue-50'
                    : vibe.id === 'innovative-forward'
                    ? 'bg-green-50'
                    : vibe.id === 'warm-accessible'
                    ? 'bg-pink-50'
                    : 'bg-gray-50'}"
                >
                  <!-- Header -->
                  <div class="flex justify-between items-center mb-4">
                    <div
                      class="font-semibold text-sm {vibe.id === 'modern-minimal'
                        ? 'text-slate-800'
                        : vibe.id === 'bold-executive'
                        ? 'text-orange-800'
                        : vibe.id === 'refined-luxury'
                  ...
                  <div class="text-center py-4">
                    <h3
                      class="font-semibold text-sm mb-2 {vibe.id ===
                      'modern-minimal'
                        ? 'text-slate-800'
                        : vibe.id === 'bold-executive'
                        ? 'text-orange-800'
                    ...
                    <div
                      class="inline-block px-3 py-1.5 rounded text-xs font-medium text-white
                              {vibe.id === 'modern-minimal'
                        ? 'bg-orange-500'
                        : vibe.id === 'bold-executive'
                        ? 'bg-orange-600'
                        : vibe.id === 'refined-luxury'
                  ...
                  <div class="grid grid-cols-3 gap-2 mt-4">
                    {#each Array(3) as _}
                      <div class="bg-white p-2 rounded">
                        <div
                          class="w-full h-2 rounded mb-1 {vibe.id ===
                          'modern-minimal'
                            ? 'bg-slate-200'
                            : vibe.id === 'bold-executive'
                            ? 'bg-orange-200'
                            ...
                            : 'bg-gray-200'}"
                        />
                        <div class="w-2/3 h-1 bg-gray-200 rounded" />
                      </div>
                    {/each}
                  </div>
                </div>
              ...
              <div class="flex items-start gap-4">
                <div
                  class="flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center transition-colors duration-300
                           {selected.includes(vibe.id)
                    ? 'bg-orange-100'
                    : vibe.id === 'modern-minimal'
                    ? 'bg-slate-50'
            ...
            {#if hoveredVibe === vibe.id && !selected.includes(vibe.id)}
              <div
                class="absolute inset-0 bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl opacity-50 pointer-events-none"
  ...
  {#if selected.length > 0}
    <div
      class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl p-8 max-w-4xl mx-auto border border-orange-200 mb-8"
    ...
    >
      ...
      <div class="flex flex-wrap gap-4 justify-center mb-6">
        {#each selectedVibes as vibe, i}
          <div
            class="flex items-center gap-3 px-6 py-3 bg-white rounded-xl shadow-sm border border-orange-200"
          >
            <div
              class="w-3 h-3 rounded-full"
              style="background-color: {vibe.accentColor}"
            />
  ...
  <div class="text-center">
    ...
    {:else}
      <div class="space-y-3">
        <p class="text-gray-600 font-light">
          Please select at least one aesthetic direction
        </p>
        <p class="text-sm text-gray-500">
          Your selection will guide all subsequent design decisions
        </p>
      </div>
    {/if}
  </div>
