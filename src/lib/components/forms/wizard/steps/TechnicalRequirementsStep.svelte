<script lang="ts">
  ...
  const hostingOptions = [
    {
      name: "You handle hosting (recommended)",
      icon: "🏆",
      description: "We manage everything for you",
    },
    {
      name: "I have existing hosting",
      icon: "🏠",
      description: "Use your current hosting provider",
    },
    {
      name: "Need hosting recommendations",
      icon: "💡",
      description: "Help me choose the best option",
    },
    {
      name: "Premium managed hosting",
      icon: "⭐",
      description: "High-performance managed solution",
    },
    {
  ...
  const domainOptions = [
    {
      name: "I own my domain",
      icon: "✅",
      description: "Already have a domain name",
    },
    {
      name: "Need help purchasing domain",
      icon: "🛒",
      description: "Help me buy a new domain",
    },
    {
      name: "Need domain transfer",
      icon: "🔄",
      description: "Move existing domain",
    },
    {
      name: "Multiple domains needed",
      icon: "🌐",
      description: "Need several domain names",
    },
  ];
  ...
  const analyticsOptions = [
    {
      ...
      icon: "📊",
      description: "Standard web analytics",
    },
    {
      name: "Advanced User Tracking",
      icon: "👥",
      description: "Detailed user behavior",
    },
    {
      name: "Conversion Tracking",
      icon: "🎯",
      description: "Track goals and conversions",
    },
    {
      name: "Heat Maps",
      icon: "🔥",
      description: "Visual user interaction maps",
    },
      ...
      {#each hostingOptions as option}
        <button
          ...
          class="relative p-4 border-2 rounded-lg text-left transition-all hover:shadow-md
            {formData.hostingPreference === option.name
    ...
    <div class="grid md:grid-cols-2 gap-4">
      {#each domainOptions as option}
        <button
          ...
          class="relative p-4 border-2 rounded-lg text-left transition-all hover:shadow-md
            {formData.domainStatus === option.name
            ? 'border-accent-600 bg-accent-50'
            : 'border-gray-200 hover:border-gray-300'}"
        >
      ...
      {#each analyticsOptions as option}
        <button
          ...
          class="p-3 border rounded-lg text-left transition-all flex items-start gap-3 hover:shadow-sm
            {formData.analyticsRequirements.includes(option.name)
            ? 'border-accent-600 bg-accent-50'
    ...
    <div class="grid md:grid-cols-2 gap-3">
      {#each securityOptions as option}
        <button
          ...
          class="p-3 border rounded-lg text-left transition-all flex items-start gap-3 hover:shadow-sm
            {formData.securityRequirements.includes(option.name)
            ? 'border-accent-600 bg-accent-50'
            : 'border-gray-300 hover:border-gray-400'}"
        >
          <div
