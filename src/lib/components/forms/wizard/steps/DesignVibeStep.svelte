<script lang="ts">
  ...
  import { Palette, Sparkles, Zap, Crown, Heart, Briefcase, Leaf, Coffee } from 'lucide-svelte';
...
<div class="space-y-8" in:fade={{ duration: 300 }}>
  ...
  <div class="text-center">
    <div class="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl mb-6 shadow-xl">
      <Palette size={40} class="text-white" />
    </div>
    
    <div class="mb-8">
      <p class="text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed">
        Choose 1-2 vibes that resonate with your brand. This will guide every design decision we make together.
      </p>
    </div>
  </div>
  ...
  <div class="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
    {#each vibes as vibe}
      <button
        ...
        in:scale={{ duration: 300, delay: Math.min(vibes.indexOf(vibe) * 100, 700) }}
      >
        <!-- Selection indicator -->
        {#if selected.includes(vibe.id)}
          <div class="absolute -top-2 -right-2 w-8 h-8 bg-purple-500 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg z-10"
        ...
        <!-- Gradient background -->
        <div class="absolute inset-0 bg-gradient-to-br {vibe.gradient} opacity-10 rounded-2xl"></div>
        ...
        <div class="relative z-10">
          ...
          <div class="flex items-start gap-4 mb-4">
            <div class="flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center" 
              ...
              <svelte:component this={vibe.icon} size={24} style="color: {vibe.accentColor}" />
            </div>
            
            <div class="flex-1 text-left">
              <h3 class="text-xl font-bold text-gray-900 mb-1">{vibe.name}</h3>
              <p class="text-sm text-gray-600 leading-relaxed">{vibe.description}</p>
            </div>
          </div>
          
          <!-- Mood indicator -->
          <div class="mb-4">
            <span class="inline-block px-3 py-1 bg-gray-100 rounded-full text-sm font-medium text-gray-700">
              {vibe.mood}
            </span>
          </div>
          ...
          <div class="mt-4 p-3 rounded-xl border border-gray-200 bg-white">
            <div class="mini-preview {vibe.preview.bg} p-3 rounded-lg">
              ...
              <div class="flex gap-1">
                <div class="{vibe.preview.accent} w-4 h-1 rounded"></div>
                <div class="{vibe.preview.accent} w-2 h-1 rounded opacity-60"></div>
                <div class="{vibe.preview.accent} w-3 h-1 rounded opacity-40"></div>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-wrap gap-2 mt-4">
            {#each vibe.tags as tag}
              <span class="px-2 py-1 bg-white border border-gray-200 rounded-lg text-xs font-medium text-gray-600">
                {tag}
              </span>
            {/each}
          </div>
          ...
          {#if hoveredVibe === vibe.id}
            <div class="absolute inset-0 bg-gradient-to-br {vibe.gradient} opacity-5 rounded-2xl pointer-events-none"
  ...
  {#if selected.length > 0}
    <div class="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-6 max-w-4xl mx-auto border border-purple-200"
      ...
      <div class="website-mockup bg-white rounded-xl shadow-lg overflow-hidden mb-6">
        <!-- Browser bar -->
        ...
        {#if selectedVibes.length > 0}
          ...
          <div class="website-content {primaryVibe.preview.bg} {primaryVibe.preview.text} p-8">
            <!-- Header -->
            <div class="flex justify-between items-center mb-8">
              <div class="{primaryVibe.preview.font} text-xl">
                Your Company
              </div>
            ...
            <div class="text-center py-12">
              ...
              <div class="{primaryVibe.preview.accent} {primaryVibe.preview.bg === 'bg-white' ? 'text-white' : primaryVibe.preview.text} px-6 py-3 rounded-lg inline-block {primaryVibe.preview.font}">
                Get Started
              </div>
            </div>
            ...
            {#if secondaryVibe}
              <div class="grid grid-cols-3 gap-6 mt-12">
                {#each Array(3) as _, i}
                  <div class="text-center p-4 rounded-lg {secondaryVibe.preview.bg} {secondaryVibe.preview.text} bg-opacity-20">
                    <div class="{secondaryVibe.preview.accent} w-12 h-12 rounded-lg mx-auto mb-3 bg-opacity-80"></div>
            ...
            {:else}
              <div class="grid grid-cols-3 gap-6 mt-12">
                {#each Array(3) as _, i}
                  <div class="text-center p-4 rounded-lg bg-opacity-10 {primaryVibe.preview.accent}">
                    <div class="{primaryVibe.preview.accent} w-12 h-12 rounded-lg mx-auto mb-3"></div>
                    <div class="font-medium">Feature {i + 1}</div>
                    <div class="text-sm opacity-75">Description here</div>
                  </div>
                {/each}
      ...
      <div class="flex flex-wrap gap-3 mb-4 justify-center">
        {#each selectedVibes as vibe}
          <div class="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-purple-200">
            <svelte:component this={vibe.icon} size={16} style="color: {vibe.accentColor}" />
      ...
      <div class="text-center">
        {#if selected.length > 1}
          <p class="text-sm text-gray-600">
            This unique combination blends <strong>{selectedVibes[0]?.name.toLowerCase()}</strong> with 
            <strong>{selectedVibes[1]?.name.toLowerCase()}</strong> for a distinctive personality.
          </p>
        {:else}
          <p class="text-sm text-gray-600">
            Your site will have a distinctly <strong>{selectedVibes[0]?.name.toLowerCase()}</strong> feeling throughout.
          </p>
        {/if}
      </div>
    </div>
  ...
  <div class="text-center pt-6">
    ...
    {#if selected.length === 0}
      <p class="text-sm text-gray-500 mt-3">Choose at least one vibe to continue</p>
    {:else}
