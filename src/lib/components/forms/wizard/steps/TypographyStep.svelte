...
<div class="mb-4">
  <p class="text-sm text-gray-600">
    Typography sets the tone for your entire website. Choose a style that
  ...
  {#each typographyOptions as option}
    ...
    >
      ...
      {#if animatingOption === option.id || isSelected(option.id)}
        ...
        >
          <svg
            class="w-5 h-5 text-orange-600 font-bold"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      <div class="space-y-4 relative z-10">
        ...
        <div class="mb-4">
          <div
            class="text-4xl {option.fontClass} mb-2 {isSelected(option.id)
          ...
          <div
            class="text-sm {option.fontClass} {isSelected(option.id)
              ? 'text-white/90 drop-shadow-sm'
              : 'text-gray-600'} transition-colors duration-300 leading-relaxed"
          >
            {option.sample}
          </div>
        </div>
        ...
        <div>
          <h3
            class="font-bold {isSelected(option.id)
              ? 'text-white drop-shadow-sm'
              : 'text-gray-900'} text-lg mb-2 transition-colors duration-300"
          >
            {option.name}
          </h3>
          <p
            class="text-sm font-medium {isSelected(option.id)
              ? 'text-white/90 drop-shadow-sm'
              : 'text-gray-600'} mb-3 transition-colors duration-300"
          >
            {option.description}
          </p>
          <p
            class="text-xs {isSelected(option.id)
              ? 'text-white/80 drop-shadow-sm'
              : 'text-gray-500'} italic transition-colors duration-300"
          >
            {option.example}
          </p>
        </div>
