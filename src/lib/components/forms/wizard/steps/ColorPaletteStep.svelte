<script lang="ts">
  ...
  $: recommendedPalettes = designMood.length > 0
    ? colorPalettes.filter(p => 
        p.moods?.some(mood => designMood.includes(mood))
      )
    : colorPalettes;
  
  $: displayPalettes = recommendedPalettes.length > 0 ? recommendedPalettes : colorPalettes;
    ...
    <div class="grid grid-cols-2 gap-4">
      {#each displayPalettes as palette}
        ...
        >
          {#if designMood.length > 0 && palette.moods?.some(mood => designMood.includes(mood))}
            <span class="absolute top-2 right-2 px-1.5 py-0.5 bg-green-100 text-green-700 text-xs rounded-md z-10 font-medium">
              ✓ Recommended
            </span>
          {/if}
          ...
          <div class="space-y-3 relative z-10">
            <div class="flex gap-1 justify-center">
              {#each palette.colors as color}
                <div 
                  class="w-12 h-12 rounded-lg shadow-sm {selected === palette.id ? 'ring-2 ring-white/50' : ''}"
            ...
            <div>
              <h3 class="font-bold {selected === palette.id ? 'text-white drop-shadow-sm' : 'text-gray-900'} transition-colors duration-300">
                {palette.name}
              </h3>
              {#if palette.description}
                <p class="text-sm font-medium {selected === palette.id ? 'text-white drop-shadow-sm' : 'text-gray-600'} mt-1 transition-colors duration-300">
                  {palette.description}
                </p>
              {/if}
            </div>
      ...
      >
        <div class="space-y-3">
          <div class="flex gap-1 justify-center">
            {#each Array(4) as _, i}
              <div class="w-12 h-12 rounded-lg border-2 border-dashed border-gray-300 bg-gray-100 flex items-center justify-center">
    ...
    <div in:fade={{ duration: 300 }} class="space-y-6">
      <div class="text-center">
        ...
        <p class="text-gray-600">
          Choose 4 colors that represent your brand
        </p>
      </div>
