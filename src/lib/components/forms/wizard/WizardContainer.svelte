<script lang="ts">
  ...
  const primaryGoalOptions = [
    'Generate Leads', 'Increase Sales', 'Build Brand Awareness', 'Improve Customer Service',
    'Streamline Operations', 'Showcase Portfolio', 'Educate Audience', 'Build Community'
  ];

  const successMetricOptions = [
    'Website Traffic', 'Lead Generation', 'Online Sales', 'User Engagement',
    'Conversion Rate', 'Brand Recognition', 'Customer Satisfaction', 'Time on Site'
  ];
  ...
  const platformTypes = [
    'Simple Website', 'Web Application', 'E-commerce Platform', 'SaaS Solution',
    'Membership Portal', 'Booking System', 'Learning Management System', 'Custom Platform'
  ];

  const coreFeatureOptions = [
    'Contact Forms', 'Email Newsletter', 'Social Media Integration', 'Search Functionality',
    'User Registration', 'Content Management', 'Mobile App', 'Multi-language Support'
  ];

  const advancedFeatureOptions = [
    'E-commerce/Shopping Cart', 'Online Booking/Scheduling', 'User Dashboards', 'Payment Processing',
    'Live Chat/Support', 'Advanced Analytics', 'API Integrations', 'Custom Workflows',
    'Real-time Notifications', 'File Upload/Management', 'Advanced Search/Filtering', 'Custom Reporting'
  ];

  const integrationOptions = [
    'CRM (Salesforce, HubSpot)', 'Email Marketing (Mailchimp, Constant Contact)', 
    'Payment (Stripe, PayPal)', 'Analytics (Google Analytics, Mixpanel)',
    'Social Media APIs', 'Accounting Software', 'Inventory Management', 'Custom APIs'
  ];
  ...
  const typographyOptions = [
    'Clean & Modern Sans-serif', 'Professional Serif', 'Creative Display Fonts',
    'Tech/Code-inspired', 'Handwritten/Script', 'Bold & Impactful', 'Let you choose'
  ];

  const visualStyleOptions = [
    'Photo-heavy/Visual storytelling', 'Illustration-based', 'Icon & graphic-focused',
    'Video & animation', 'Text & typography-focused', 'Mixed media approach'
  ];

  // Step 5 Options
  const pageStructureOptions = [
    'Homepage', 'About Us', 'Services/Products', 'Portfolio/Case Studies', 'Blog/News',
    'Contact', 'Pricing', 'FAQ', 'Team', 'Testimonials', 'Privacy Policy', 'Terms of Service'
  ];

  const contentSectionOptions = [
    'Hero/Banner Section', 'Features Overview', 'Testimonials/Reviews', 'Call-to-Action Sections',
    'Product/Service Galleries', 'Team Bios', 'Company Timeline', 'Process/How It Works',
    'Statistics/Metrics', 'FAQ Section', 'Contact Information', 'Social Proof'
  ];
  ...
  const mediaRequirementOptions = [
    'Professional Photography', 'Stock Photos', 'Custom Graphics/Illustrations',
    'Video Content', 'Infographics', 'Icons & Logos', 'Interactive Elements'
  ];

  // Step 6 Options
  const keyInteractionOptions = [
    'Contact Form Submissions', 'Product Purchases', 'Service Bookings', 'Newsletter Signups',
    'Content Downloads', 'User Registration', 'Social Sharing', 'Live Chat Engagement'
  ];

  const responsiveRequirements = [
    'Mobile-first Design', 'Tablet Optimization', 'Desktop Experience', 'Touch-friendly Interface',
    'Fast Mobile Loading', 'App-like Mobile Experience'
  ];

  const performanceRequirements = [
    'Lightning Fast Loading', 'SEO Optimization', 'Accessibility Compliance', 'Security Features',
    'Scalability Planning', 'Browser Compatibility', 'Performance Monitoring'
  ];

  // Step 7 Options
  const hostingOptions = [
    'You handle hosting (recommended)', 'I have existing hosting', 'Need hosting recommendations',
    'Premium managed hosting', 'Cloud hosting solution'
  ];

  const domainOptions = [
    'I own my domain', 'Need help purchasing domain', 'Need domain transfer', 'Multiple domains needed'
  ];

  const analyticsOptions = [
    'Google Analytics', 'Advanced User Tracking', 'Conversion Tracking', 'Heat Maps',
    'A/B Testing', 'Custom Reporting', 'Real-time Analytics'
  ];

  const securityOptions = [
    'SSL Certificate', 'Regular Backups', 'Security Monitoring', 'User Authentication',
    'Data Encryption', 'GDPR Compliance', 'PCI Compliance'
  ];
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each primaryGoalOptions as goal}
                <button
                  type="button"
                  on:click={() => formData.primaryGoals = toggleArrayOption(formData.primaryGoals, goal)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.primaryGoals.includes(goal) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.primaryGoals.includes(goal) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each successMetricOptions as metric}
                <button
                  type="button"
                  on:click={() => formData.successMetrics = toggleArrayOption(formData.successMetrics, metric)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.successMetrics.includes(metric) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.successMetrics.includes(metric) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
              ...
              {#each platformTypes as platform}
                <button
                  ...
                  class="p-4 border rounded-lg text-left transition-all
                    {formData.platformType === platform ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each coreFeatureOptions as feature}
                <button
                  type="button"
                  on:click={() => formData.coreFeatures = toggleArrayOption(formData.coreFeatures, feature)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.coreFeatures.includes(feature) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.coreFeatures.includes(feature) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each advancedFeatureOptions as feature}
                <button
                  type="button"
                  on:click={() => formData.advancedFeatures = toggleArrayOption(formData.advancedFeatures, feature)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.advancedFeatures.includes(feature) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.advancedFeatures.includes(feature) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each integrationOptions as integration}
                <button
                  type="button"
                  on:click={() => formData.integrations = toggleArrayOption(formData.integrations, integration)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.integrations.includes(integration) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.integrations.includes(integration) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
            ...
            <div class="space-y-2">
              {#each typographyOptions as typo}
                <label class="flex items-center gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50
                  {formData.typography === typo ? 'border-accent-600 bg-accent-50' : 'border-gray-200'}">
                  <input
                    type="radio"
          ...
          <div>
            ...
            <div class="space-y-2">
              {#each visualStyleOptions as style}
                <label class="flex items-center gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50
                  {formData.visualStyle === style ? 'border-accent-600 bg-accent-50' : 'border-gray-200'}">
                  <input
                    type="radio"
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each pageStructureOptions as structure}
                <button
                  type="button"
                  on:click={() => formData.pageStructure = toggleArrayOption(formData.pageStructure, structure)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.pageStructure.includes(structure) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.pageStructure.includes(structure) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each contentSectionOptions as section}
                <button
                  type="button"
                  on:click={() => formData.contentSections = toggleArrayOption(formData.contentSections, section)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.contentSections.includes(section) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.contentSections.includes(section) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each mediaRequirementOptions as requirement}
                <button
                  type="button"
                  on:click={() => formData.mediaRequirements = toggleArrayOption(formData.mediaRequirements, requirement)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.mediaRequirements.includes(requirement) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.mediaRequirements.includes(requirement) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each keyInteractionOptions as interaction}
                <button
                  type="button"
                  on:click={() => formData.keyInteractions = toggleArrayOption(formData.keyInteractions, interaction)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.keyInteractions.includes(interaction) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.keyInteractions.includes(interaction) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each responsiveRequirements as requirement}
                <button
                  type="button"
                  on:click={() => formData.responsiveRequirements = toggleArrayOption(formData.responsiveRequirements, requirement)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.responsiveRequirements.includes(requirement) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.responsiveRequirements.includes(requirement) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each performanceRequirements as requirement}
                <button
                  type="button"
                  on:click={() => formData.performanceRequirements = toggleArrayOption(formData.performanceRequirements, requirement)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.performanceRequirements.includes(requirement) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.performanceRequirements.includes(requirement) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each hostingOptions as option}
                <button
                  ...
                  class="p-4 border rounded-lg text-left transition-all
                    {formData.hostingPreference === option ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each domainOptions as option}
                <button
                  ...
                  class="p-4 border rounded-lg text-left transition-all
                    {formData.domainStatus === option ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <span class="font-medium">{option}</span>
                </button>
              {/each}
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each analyticsOptions as option}
                <button
                  type="button"
                  on:click={() => formData.analyticsRequirements = toggleArrayOption(formData.analyticsRequirements, option)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.analyticsRequirements.includes(option) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.analyticsRequirements.includes(option) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
          ...
          <div>
            ...
            <div class="grid md:grid-cols-2 gap-3">
              {#each securityOptions as option}
                <button
                  type="button"
                  on:click={() => formData.securityRequirements = toggleArrayOption(formData.securityRequirements, option)}
                  class="p-3 border rounded-lg text-left transition-all flex items-center gap-3
                    {formData.securityRequirements.includes(option) ? 'border-accent-600 bg-accent-50' : 'border-gray-300 hover:border-gray-400'}"
                >
                  <div class="w-5 h-5 border rounded 
                    {formData.securityRequirements.includes(option) ? 'bg-accent-600 border-accent-600' : 'border-gray-300'}">
