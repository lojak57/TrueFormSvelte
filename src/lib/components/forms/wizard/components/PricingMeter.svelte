<script lang="ts">
  ...
  $: showMeter =
    $wizardStore.currentStepIndex >= 9 &&
  ...
  $: enterpriseFeatureNames =
    ($currentPrice.enterpriseFeatures as string[])
      ?.map((feature) => FEATURE_DISPLAY_NAMES[feature] || feature)
      .join(", ") || "";
</script>
    ...
    <div class="max-w-4xl mx-auto px-6 py-4">
            ...
            <div class="hidden sm:block">
              <div class="flex items-center gap-2 text-sm">
                <span class="text-gray-300">
                  {$currentPrice.featureCount} feature{$currentPrice.featureCount !==
                  1
                    ? "s"
                    : ""} selected
                </span>
                {#if !$currentPrice.isEnterprise && $currentPrice.featureCount > PRICING_MODEL.includedFeatures}
                  <span class="text-accent-400 font-medium">
                    (+{$currentPrice.featureCount -
                      PRICING_MODEL.includedFeatures} premium)
                  </span>
                {/if}
              </div>
            </div>
        ...
        {:else if $currentPrice.addOns.length > 0}
          <div class="flex items-center gap-2" in:fade={{ duration: 200 }}>
            ...
            <span class="text-sm text-gray-300">
              {$currentPrice.addOns.length} add-on{$currentPrice.addOns
                .length !== 1
                ? "s"
                : ""}
            </span>
      ...
      {#if $currentPrice.featureCount > 0}
        <div class="sm:hidden mt-2 text-xs text-gray-300">
          {$currentPrice.featureCount} feature{$currentPrice.featureCount !== 1
            ? "s"
            : ""} selected
          {#if !$currentPrice.isEnterprise && $currentPrice.featureCount > PRICING_MODEL.includedFeatures}
            <span class="text-accent-400 font-medium">
      ...
      {#if $currentPrice.isEnterprise && $currentPrice.enterpriseFeatures}
        <div
          class="mt-3 p-3 bg-purple-900/30 rounded-lg border border-purple-700"
        >
          <p class="text-sm text-purple-300 font-medium mb-1">
