<script lang="ts">
  ...
  $: selectedArray = Array.isArray(selected) ? selected : (selected ? [selected] : []);
  ...
  <div class="grid gap-4" style="grid-template-columns: repeat({columns}, 1fr)">
    {#each options as option}
      <button
        ...
        class="relative {option.image ? 'p-0' : 'p-6'} border-2 rounded-xl text-left transition-all duration-300 hover:shadow-lg transform hover:scale-[1.02] overflow-hidden
      ...
      >
        ...
        {#if animatingOption === option.id || isSelected(option.id)}
          <div 
            ...
            in:scale={{ duration: multiple ? 300 : 500, start: 0, easing: (t) => t * t * (3 - 2 * t) }}
          ...
          {#if option.image}
            ...
            <div class="relative h-32 overflow-hidden rounded-t-xl">
              <img 
                src={option.image} 
                alt={option.name}
                class="w-full h-full object-cover {isSelected(option.id) ? 'brightness-110' : ''}"
              />
              <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
              <div class="absolute bottom-2 left-2 right-2">
                <div class="flex items-center gap-2 mb-1">
                  {#if option.icon}
                    <span class="text-lg filter drop-shadow-sm">{option.icon}</span>
                  {/if}
           ...
           {:else}
            ...
            {#if option.icon}
              <div class="text-4xl {isSelected(option.id) ? 'filter drop-shadow-sm' : ''}">{option.icon}</div>
            {/if}
            
            <div>
              <h3 class="font-bold {isSelected(option.id) ? 'text-white drop-shadow-sm' : 'text-gray-900'} transition-colors duration-300">
                {option.name}
              </h3>
              {#if option.description}
                <p class="text-sm font-medium {isSelected(option.id) ? 'text-white drop-shadow-sm' : 'text-gray-600'} mt-1 transition-colors duration-300">
                  {option.description}
                </p>
              {/if}
            </div>
