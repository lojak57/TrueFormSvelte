<script lang="ts">
  ...
  import type { SiteRequestFormData, StepValidationEvent } from '$lib/types/wizard.types';
  ...
  $: ({ currentStep, data: formData, isSubmitting, submitError } = $wizard.store);
...
<Card class="max-w-5xl mx-auto">
  <!-- Progress Steps -->
  <WizardProgress {steps} {currentStep} stepDescriptions={stepDescriptions} />
  ...
  <div class="min-h-96">
    ...
    {#if currentStep === 3}
      <div in:fade={{ duration: 300 }}>
        <DesignBrandingStep bind:formData on:validation={handleStepValidation} />
      </div>
    {/if}

    {#if currentStep === 4}
      <div in:fade={{ duration: 300 }}>
        <ContentStructureStep bind:formData on:validation={handleStepValidation} />
      </div>
    {/if}

    {#if currentStep === 5}
      <div in:fade={{ duration: 300 }}>
        <UserExperienceStep bind:formData on:validation={handleStepValidation} />
      </div>
    {/if}

    {#if currentStep === 6}
      <div in:fade={{ duration: 300 }}>
        <TechnicalRequirementsStep bind:formData on:validation={handleStepValidation} />
      </div>
    {/if}

    {#if currentStep === 7}
      <div in:fade={{ duration: 300 }}>
        <TimelineBudgetStep bind:formData on:validation={handleStepValidation} />
      </div>
    {/if}
  </div>
