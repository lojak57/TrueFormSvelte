<script lang="ts">
  ...
  import { Plus, Phone, Mail, Calendar, DollarSign, FileText } from 'lucide-svelte';
  import { getTrueFormOpportunities, updateOpportunityStatus } from '$lib/api/trueform';
  ...
  const columns = [
    ...
    { id: 'closed_won', title: 'Closed Won', color: 'bg-green-50 border-green-200' },
    { id: 'closed_lost', title: 'Closed Lost', color: 'bg-gray-50 border-gray-200' }
  ];
...
<div class="space-y-6">
  ...
  <div class="flex justify-between items-center">
    <div>
      <h2 class="text-2xl font-semibold text-gray-900 mb-2">🎯 TrueForm Pipeline</h2>
  ...
  {#if error}
    <div class="bg-red-50 border border-red-200 rounded-lg p-4" in:fade>
      ...
      <Button variant="outline" size="sm" on:click={loadOpportunities} class="mt-2">
        Retry
      </Button>
    </div>
  {/if}
          ...
          >
            <div class="space-y-4">
              {#each getOpportunitiesByStatus(column.id) as opportunity}
                ...
                >
                  <Card class="p-4 hover:shadow-md transition-shadow {isRecentOpportunity(opportunity.created_at) ? 'ring-2 ring-blue-200 bg-blue-50' : ''}">
                    <!-- Opportunity Header -->
                      ...
                      <div class="flex-1">
                        <div class="flex items-center gap-2">
                          ...
                          {#if isRecentOpportunity(opportunity.created_at)}
                            <span class="bg-blue-500 text-white text-xs px-2 py-1 rounded-full font-medium">NEW</span>
                          {/if}
                        </div>
                        <p class="text-xs text-gray-600 mt-1">{opportunity.company}</p>
                      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-8">
      ...
      <Card class="p-6 text-center">
        <div class="text-2xl font-bold text-green-600 mb-2">
          {formatCurrency(opportunities.filter(o => o.status === 'closed_won').reduce((sum, o) => sum + o.value, 0))}
        </div>
      ...
      <Card class="p-6 text-center">
        <div class="text-2xl font-bold text-orange-600 mb-2">
          {Math.round(opportunities.filter(o => o.status !== 'closed_lost' && o.status !== 'closed_won').reduce((sum, o) => sum + (o.value * o.probability / 100), 0))}
        </div>
...
<div class="fixed bottom-6 right-6 z-50">
  ...
  >
    <FileText size={24} />
    <span class="hidden group-hover:block text-sm font-medium">New Proposal</span>
  </button>
</div>
