<script lang="ts">
  ...
  const columns = [
    { id: 'discovery', title: 'Discovery', opportunities: [
      { id: 1, company: 'Acme Corp', value: 99, contact: 'john@acme.com' },
      { id: 2, company: 'TechStart', value: 199, contact: 'sarah@techstart.com' }
    ]},
    { id: 'proposal', title: 'Proposal', opportunities: [
    ...
    { id: 'closed_won', title: 'Closed Won', opportunities: [
        ...
        {#each column.opportunities as opportunity}
          <Card class="p-4 cursor-move hover:shadow-md transition-shadow">
            <div class="flex justify-between items-start mb-2">
              ...
              <span class="text-accent-600 font-semibold">${opportunity.value}</span>
            </div>
            <p class="text-sm text-gray-600">{opportunity.contact}</p>
          </Card>
        {/each}

        <Button variant="ghost" class="w-full justify-start text-gray-500 border-2 border-dashed border-gray-200">
...
<div class="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-lg">
  <p class="text-blue-700 text-sm">
    <strong>Note:</strong> Drag-and-drop functionality will be implemented using dnd-kit in the next phase.
  </p>
</div> 