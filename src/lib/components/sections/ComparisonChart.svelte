<script lang="ts">
  import { Check, X, Zap, Award } from "lucide-svelte";
  ...
  const competitors = [
    {
      ...
      price: "$20-40/mo",
      totalCost: "$240-480/year",
      features: [
        { name: "Custom Design", value: false },
        { name: "Professional Look", value: false },
        { name: "Your Time Required", value: "50+ hours" },
        { name: "Performance", value: false },
        { name: "SEO Optimization", value: false },
        { name: "Human Support", value: false },
        { name: "Hosting Included", value: true },
        { name: "Mobile Optimized", value: "basic" }
      ]
    },
    {
      name: "Freelancers",
      subtitle: "Upwork, Fiverr, etc.",
      price: "$500-2000",
      totalCost: "$500-2000 + hosting",
      features: [
        { name: "Custom Design", value: "maybe" },
        { name: "Professional Look", value: "maybe" },
        { name: "Your Time Required", value: "20+ hours" },
        { name: "Performance", value: "maybe" },
        { name: "SEO Optimization", value: "maybe" },
        { name: "Human Support", value: false },
        { name: "Hosting Included", value: false },
        { name: "Mobile Optimized", value: "maybe" }
      ]
    },
    {
      name: "TrueForm",
      ...
      price: "$999",
      totalCost: "$999 (Year 1 hosting FREE)",
      isUs: true,
      features: [
        { name: "Custom Design", value: true },
        { name: "Professional Look", value: true },
        { name: "Your Time Required", value: "2 hours" },
        { name: "Performance", value: true },
        { name: "SEO Optimization", value: true },
        { name: "Human Support", value: true },
        { name: "Hosting Included", value: true },
        { name: "Mobile Optimized", value: true }
      ]
    },
    {
      name: "Agencies",
      subtitle: "Traditional dev shops",
      price: "$5000-15000",
      totalCost: "$5000-15000 + hosting",
      features: [
        { name: "Custom Design", value: true },
        { name: "Professional Look", value: true },
        { name: "Your Time Required", value: "40+ hours" },
        { name: "Performance", value: true },
        { name: "SEO Optimization", value: true },
        { name: "Human Support", value: "during project" },
        { name: "Hosting Included", value: false },
        { name: "Mobile Optimized", value: true }
      ]
    }
  ...
  function getFeatureDisplay(value: boolean | string) {
    if (value === true) return { icon: Check, class: "text-green-600", text: "✓" };
    if (value === false) return { icon: X, class: "text-red-500", text: "✗" };
    if (value === "maybe") return { icon: null, class: "text-yellow-600", text: "Maybe" };
    if (value === "basic") return { icon: null, class: "text-yellow-600", text: "Basic" };
    if (value === "during project") return { icon: null, class: "text-yellow-600", text: "During project only" };
    return { icon: null, class: "text-gray-700", text: value };
  }
</script>
...
<section class="py-20 bg-gradient-to-br from-gray-50 via-white to-gray-50">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="text-center mb-16">
      <div class="inline-flex items-center px-4 py-2 mb-6 bg-accent-100 border border-accent-200 rounded-full text-accent-700 text-sm font-medium">
        <Award size={16} class="mr-2" />
        The Smart Choice Comparison
      </div>
      <h2 class="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
        What You Get vs <span class="text-accent-600">The Competition</span>
      </h2>
      <p class="text-xl text-gray-600 max-w-3xl mx-auto">
        See exactly why TrueForm delivers more value than DIY builders, freelancers, or expensive agencies.
      </p>
    </div>
      ...
      <div class="inline-block min-w-full align-middle">
        ...
        <div class="hidden md:grid grid-cols-1 md:grid-cols-4 gap-6 lg:gap-4">
          {#each competitors as competitor}
            <div class="relative">
              {#if competitor.isUs}
                <!-- TrueForm highlight card -->
                <div class="absolute -inset-1 bg-gradient-to-r from-accent-600 to-accent-400 rounded-2xl blur opacity-75"></div>
                <div class="relative bg-white rounded-2xl border-2 border-accent-200 shadow-xl">
                  <div class="p-6">
                    <div class="text-center mb-6">
                      <div class="inline-flex items-center px-3 py-1 bg-accent-600 text-white text-xs font-semibold rounded-full mb-3">
                        <Zap size={12} class="mr-1" />
                        BEST VALUE
                      </div>
                      <h3 class="text-2xl font-bold text-gray-900">{competitor.name}</h3>
                      <p class="text-sm text-gray-600 mb-3">{competitor.subtitle}</p>
                      <div class="text-3xl font-bold text-accent-600">{competitor.price}</div>
                      <p class="text-sm text-gray-600">{competitor.totalCost}</p>
                    </div>
                  </div>
                </div>
              {:else}
                <!-- Regular competitor card -->
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                  <div class="p-6">
                    <div class="text-center mb-6">
                      <h3 class="text-xl font-semibold text-gray-900">{competitor.name}</h3>
                      <p class="text-sm text-gray-500 mb-3">{competitor.subtitle}</p>
                      <div class="text-2xl font-bold text-gray-900">{competitor.price}</div>
                      <p class="text-sm text-gray-500">{competitor.totalCost}</p>
                    </div>
                  </div>
                </div>
        ...
        <div class="text-center mt-8 md:mt-12 px-4">
          ...
          <p class="text-sm text-gray-500 mt-3">Ready in 7 days. First year hosting included.</p>
        </div>
      </div>
    </div>
  </div>
