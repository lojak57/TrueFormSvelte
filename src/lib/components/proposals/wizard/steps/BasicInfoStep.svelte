<script lang="ts">
  ...
  $: filteredContacts = contacts.filter(c => c.company_id === selectedCompanyId);
...
<div class="space-y-6">
  ...
  <div class="text-center mb-8">
    ...
    <p class="tf-text-muted">Choose the company and give your proposal a descriptive title</p>
  </div>
  ...
  <div class="tf-card">
    <div class="tf-card-header">
      ...
      <p class="tf-text-muted text-sm">Choose the company this proposal is for</p>
    </div>
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              {#if selectedCompany.billing_city && selectedCompany.billing_state}
                <p class="text-sm text-blue-600">{selectedCompany.billing_city}, {selectedCompany.billing_state}</p>
              {/if}
            </div>
          </div>
        </div>
  ...
  {#if selectedCompanyId && filteredContacts.length > 0}
    <div class="tf-card">
      <div class="tf-card-header">
        ...
        <p class="tf-text-muted text-sm">Select the main contact for this proposal (optional)</p>
      </div>
      <div class="tf-card-body">
        <select 
          class="tf-input tf-select"
          ...
          {#if selectedContact}
            <div class="mt-4 p-4 bg-green-50 rounded-lg">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="font-medium text-green-900">{selectedContact.first_name} {selectedContact.last_name}</p>
                  {#if selectedContact.email}
                    <p class="text-sm text-green-700">{selectedContact.email}</p>
                  {/if}
                  {#if selectedContact.title}
                    <p class="text-sm text-green-600">{selectedContact.title}</p>
                  {/if}
                </div>
              </div>
            </div>
  ...
  <div class="tf-card">
    <div class="tf-card-header">
      ...
      <p class="tf-text-muted text-sm">Give your proposal a clear, descriptive title</p>
    </div>
    <div class="tf-card-body">
      <input 
        type="text" 
