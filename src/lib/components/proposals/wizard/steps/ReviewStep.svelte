  ...
  <div class="tf-grid tf-grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-6">
      ...
      <div class="tf-card tf-card-elevated">
        <div class="tf-card-header bg-gradient-to-r from-primary-600 to-primary-700 text-white">
          <div class="flex items-center justify-between">
            <div>
              <h4 class="text-xl font-bold">{proposalTitle}</h4>
              <p class="text-primary-100 text-sm mt-1">Proposal #{proposalNumber}</p>
            </div>
                ...
                <div class="space-y-2">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {#if company.website}
                        <div class="text-sm text-gray-600">{company.website}</div>
                      {/if}
                    </div>
                  </div>
      ...
      <div class="tf-card">
        <div class="tf-card-header">
          ...
          <p class="tf-text-muted text-sm">{lineItems.length} services • {formatCurrency(subtotal)} subtotal</p>
        </div>
        <div class="tf-card-body">
          {#if lineItems.length > 0}
            <div class="space-y-4">
              {#each lineItems as item, index}
                <div class="flex items-start justify-between py-3 {index < lineItems.length - 1 ? 'border-b border-gray-100' : ''}">
                  <div class="flex-1">
                  ...
                  <div class="ml-6 text-right">
                    <div class="font-semibold text-lg">{formatCurrency(item.total)}</div>
                  </div>
                </div>
              {/each}
            </div>
      ...
      <!-- Next Steps -->
      <div class="tf-card bg-gradient-to-br from-green-50 to-emerald-50 border-green-200">
        <div class="tf-card-body">
          <div class="flex items-start gap-4">
            ...
            <div>
              <h4 class="font-semibold text-green-900 mb-2">Ready to Create Your Proposal?</h4>
              <p class="text-green-800 text-sm mb-4">
                Once created, you'll be able to download a PDF, send it to your client, and track its status. 
                You can always edit the proposal later if needed.
              </p>
              <div class="flex flex-wrap gap-2 text-sm">
                <span class="px-2 py-1 bg-green-100 text-green-700 rounded-md">✓ PDF Generation</span>
                <span class="px-2 py-1 bg-green-100 text-green-700 rounded-md">✓ Email Integration</span>
                <span class="px-2 py-1 bg-green-100 text-green-700 rounded-md">✓ Status Tracking</span>
                <span class="px-2 py-1 bg-green-100 text-green-700 rounded-md">✓ Easy Editing</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="tf-card">
          ...
          <div class="tf-card-body space-y-4">
            ...
            <div class="flex justify-between items-center text-lg">
              ...
              <span class="font-bold text-primary-600">{formatCurrency(total)}</span>
            </div>
          </div>
        </div>
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                <p class="text-blue-800 text-sm mt-1">
                  This is how your proposal will appear to clients. You can customize the design and branding in settings.
                </p>
              </div>
            </div>
          </div>
