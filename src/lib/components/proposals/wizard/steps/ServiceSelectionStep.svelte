<script lang="ts">
  import { SERVICE_CATEGORIES, getPopularServices, searchServices } from '$lib/data/serviceTemplates';
...
<div class="space-y-8">
  ...
  <div class="text-center">
    ...
    <p class="tf-text-muted">Select from our template services or add custom items</p>
  </div>
  ...
  {#if selectedServices.length > 0}
    <div class="tf-card">
      <div class="tf-card-header">
        <h4 class="tf-heading-4">Selected Services ({selectedServices.length})</h4>
      </div>
      <div class="tf-card-body">
        <div class="flex flex-wrap gap-2">
          {#each selectedServices as service}
            <span class="inline-flex items-center gap-2 px-3 py-2 bg-primary-100 text-primary-700 rounded-lg text-sm">
              {service.name}
              <button 
  ...
  <div class="tf-card">
    <div class="tf-card-header">
      ...
      <p class="tf-text-muted text-sm">Create a custom line item not covered by our templates</p>
    </div>
