<script lang="ts">
  ...
  $: canProceedFromStep4 = lineItems.length > 0 && proposalData.title && proposalData.company_id;
  ...
  function updateLineItemsFromServices() {
    ...
    const customItems = lineItems.filter(item => 
      !SERVICE_TEMPLATES.some(service => service.id === item.id)
    );
  ...
  function handleLineItemRemove(event: CustomEvent<string>) {
    ...
    selectedServices = selectedServices.filter(service => service.id !== itemId);
  }
    ...
    <div class="tf-animate-fade-in">
      ...
      {:else if currentStep === 2}
        <ServiceSelectionStep 
          {selectedServices}
          customLineItems={lineItems.filter(item => !SERVICE_TEMPLATES.some(s => s.id === item.id))}
