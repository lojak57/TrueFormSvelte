<script lang="ts">
  ...
  function formatPrice(price: number, unit: string) {
    return `$${price.toLocaleString()}${unit === 'hour' ? '/hr' : unit === 'month' ? '/mo' : unit === 'year' ? '/yr' : ''}`;
  }
</script>

<div 
  class="tf-card tf-card-interactive {isSelected ? 'ring-2 ring-primary-500 bg-primary-50' : ''}"
  ...
  <div class="p-6">
    ...
    <div class="flex items-start justify-between mb-4">
      <div class="flex-1">
        ...
        <p class="tf-text-muted text-sm leading-relaxed">{service.description}</p>
      </div>
