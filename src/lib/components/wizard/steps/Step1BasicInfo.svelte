<script lang="ts">
  ...
  function validatePhone(phone: string): boolean {
    return phone.length === 0 || /^[\+]?[1-9][\d]{0,15}$/.test(phone.replace(/[\s\-\(\)]/g, ''));
  }
  
  // Reactive validation
  $: emailError = data.contactEmail && !validateEmail(data.contactEmail) ? 'Please enter a valid email address' : '';
  $: phoneError = data.contactPhone && !validatePhone(data.contactPhone) ? 'Please enter a valid phone number' : '';
</script>

<div class="step-content space-y-6">
  <div class="intro-text">
    <p class="text-gray-700 mb-6">
      Let's start with some basic information about your company and project. This helps us understand your needs better.
    </p>
  </div>
