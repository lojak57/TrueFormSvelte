...
const backupFiles = danger.git.created_files.concat(danger.git.modified_files)
  .filter(file => 
...
if (backupFiles.length > 0) {
  fail(`❌ Backup files detected. Please remove these files and rely on Git for version control:\n${backupFiles.map(f => `- ${f}`).join('\n')}`);
}

// Check for oversized files (Phase 3 enforcement)
const OVERSIZED_LIMIT = 400; // lines
const oversizedFiles = danger.git.created_files.concat(danger.git.modified_files)
...
if (oversizedFiles.length > 0) {
  warn(`⚠️ These files may exceed the 200/300/400 LOC limits defined in our architecture guidelines:\n${oversizedFiles.map(f => `- ${f}`).join('\n')}`);
}

// Check for TODO comments in PR title or description
const todoPattern = /TODO|FIXME|HACK|XXX/i;
const prHasTodos = todoPattern.test(danger.github.pr.title) || 
                   todoPattern.test(danger.github.pr.body || '');

if (prHasTodos) {
  warn('⚠️ TODO/FIXME mentioned in PR. Consider creating GitHub issues for tracking.');
}
...
const missingTests = hasNewComponents.filter(component => {
  ...
  return !danger.git.created_files.includes(testFile) && !danger.git.modified_files.includes(testFile);
});

if (missingTests.length > 0) {
  warn(`⚠️ New components without corresponding tests:\n${missingTests.map(f => `- ${f}`).join('\n')}`);
}
