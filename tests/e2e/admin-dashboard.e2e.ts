...
test.describe('Admin Dashboard', () => {
  ...
  test('dashboard loads and displays correctly', async ({ page }) => {
    ...
    await expect(page.locator('text=Dashboard, text=Opportunities, text=Admin')).toBeVisible();
  });
  ...
  test('can view opportunity details', async ({ page }) => {
    ...
    const opportunityLinks = page.locator('a[href*="/admin/opportunities/"], .opportunity-item, tr td:first-child a');
    ...
    if (await opportunityLinks.count() > 0) {
      ...
      await expect(page.locator('text=Details, text=Contact, text=Project')).toBeVisible();
    }
  });
  ...
  test('can update opportunity status', async ({ page }) => {
    ...
    const statusSelects = page.locator('select[name*="status"], .status-dropdown');
  ...
  test('can delete opportunities', async ({ page }) => {
    ...
    const deleteButtons = page.locator('button:has-text("Delete"), .delete-btn, [aria-label*="delete"]');
    ...
    if (await deleteButtons.count() > 0) {
      ...
      await expect(page.locator('text=confirm, text=sure, text=delete')).toBeVisible();
  ...
  test('admin navigation works correctly', async ({ page }) => {
    ...
    if (await navLinks.count() > 0) {
      // Test dashboard link
      const dashboardLink = page.locator('a:has-text("Dashboard"), a[href*="/admin"]').first();
      ...
      const opportunitiesLink = page.locator('a:has-text("Opportunities"), a[href*="/opportunities"]');
  ...
  test('can export data', async ({ page }) => {
    ...
    const exportButton = page.locator('button:has-text("Export"), .export-btn, a:has-text("Download")');
...
test.describe('Admin Dashboard - Authentication', () => {
  test('redirects to login when not authenticated', async ({ page }) => {
    ...
    await expect(page.locator('text=login, text=sign in, input[type="password"]')).toBeVisible();
  });
  ...
  test('can login with valid credentials', async ({ page }) => {
    ...
    if (await emailInput.count() > 0 && await passwordInput.count() > 0) {
      ...
      await expect(page.locator('text=Dashboard, text=Admin, text=Opportunities')).toBeVisible();
    }
  });
  ...
  test('can logout successfully', async ({ page }) => {
    ...
    const logoutButton = page.locator('button:has-text("Logout"), a:has-text("Sign Out"), .logout-btn');
...
test.describe('Admin Dashboard - Mobile', () => {
  ...
  test('admin interface is mobile responsive', async ({ page }) => {
    ...
    const mobileMenuButton = page.locator('button[aria-label="Menu"], .mobile-menu-btn');
    
    if (await mobileMenuButton.count() > 0) {
      await mobileMenuButton.click();
      
      // Verify mobile menu opens
      await expect(page.locator('text=Dashboard, text=Opportunities')).toBeVisible();
    }
  });
...
test.describe('Admin Dashboard - Performance', () => {
  ...
  test('large datasets render efficiently', async ({ page }) => {
    ...
    const paginationControls = page.locator('.pagination, .page-nav, button:has-text("Next")');
